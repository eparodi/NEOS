...
; -----------------------------------------------------------------------------
;	Writes data to the PCI.
;	Parameters:
;		-rdi: bus number. (bl)
;		-rsi: device/function number. (cl)
;		-rdx: register number. (dl)
;		-rcx: data to write. (ecx)
; -----------------------------------------------------------------------------
; Data form is binary 10000000 bbbbbbbb dddddfff rrrrrr00
_pci_write_reg:
	push rbx
...
;A configuration cycle consists of writing to the address port to specify which
;device and register you want to access and then reading or writing the data to
;the data port.
